...
resource "aci_leaf_profile" "Leaf_Profile_dc2-leaf202" {
    ...
    name                            = "dc2-leaf202"
    relation_infra_rs_acc_port_p    = [aci_leaf_interface_profile.Interface_Profile_dc2-leaf202.id]
    leaf_selector {
        name                    = "dc2-leaf202"
...
GUI Location:
 - Fabric > Access Policies > Switches > Leaf Switches > Profiles > dc2-leaf202: Leaf Selectors Policy Group: dc2-leaf202
*/
resource "aci_rest" "Leaf_Profile_dc2-leaf202_PolicyGroup_default" {
    depends_on  = [
