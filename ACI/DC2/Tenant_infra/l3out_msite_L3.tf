...
*/
resource "aci_l3_outside" "infra_l3out_msite_L3" {
    depends_on                                          = [
        data.aci_l3_domain_profile.L3Out,
        aci_tenant.infra,
        aci_vrf.overlay-1_overlay-1
    ]
    tenant_dn                                           = aci_tenant.infra.id
    description                                         = "Multi-Site L3Out"
    name                                                = "msite_L3"
    enforce_rtctrl                                      = "export"
    target_dscp                                         = "unspecified"
    relation_l3ext_rs_ectx                              = aci_vrf.overlay-1_overlay-1.id
