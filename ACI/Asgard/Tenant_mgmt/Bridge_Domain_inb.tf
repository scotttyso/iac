...
API Information:
 - Class: "fvSubnet"
 - Distinguished Name: "/uni/tn-mgmt/BD-inb/subnet-[10.92.0.254/24]"
GUI Location:
 - Tenants > mgmt > Networking > Bridge Domains > inb > Subnets
*/
resource "aci_subnet" "Tenant_mgmt_Bridge_Domain_inb_Subnet_10-92-0-0_24" {
    depends_on                          = [
        aci_tenant.Tenant_mgmt,
    ...
    ctrl                                = ["nd"]
    description                         = "Asgard Inband Subnet"
    ip                                  = "10.92.0.254/24"
    preferred                           = "no"
    scope                               = ["public"]
    virtual                             = "no"
