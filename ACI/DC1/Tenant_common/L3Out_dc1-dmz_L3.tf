...
API Information:
 - Class: "bgpExtP"
 - Distinguished Name: "uni/tn-common/out-dc1-dmz_L3/bgpExtP"
GUI Location:
 - Tenants > common > Networking > L3Outs > dc1-dmz_L3 > Enable BGP (Checkbox)
*/
resource "aci_rest" "common_l3out_dc1-dmz_L3_bgpExtP" {
    depends_on  = [
        aci_tenant.Tenant_common,
        aci_l3_outside.Tenant_common_L3Out_dc1-dmz_L3,
    ]
    path        = "/api/node/mo/uni/tn-common/out-dc1-dmz_L3/bgpExtP.json"
    class_name  = "bgpExtP"
    payload     = <<EOF
{
	"bgpExtP": {
		"attributes": {
			"dn": "uni/tn-common/out-dc1-dmz_L3/bgpExtP",
			"status": "created"
		},
		"children": []
	}
