...
class Site_Policies(object):
    ...
    def site_id(self, wb, ws, row_num, **kwargs):
        ...
        folder_list = ['Access', 'Admin', 'Fabric', 'Tenant_common', 'Tenant_infra', 'Tenant_mgmt', 'VLANs']
        ...
        if templateVars['Run_Location'] == 'Terraform_Cloud':
            ...
            workspace_dict = {}
            for folder in folder_list:
                kwargs['Working_Directory'] = 'ACI/%s/%s' % (templateVars['Site_Name'], folder)
                if not templateVars['Workspace_Prefix'] == None:
                    kwargs['Workspace_Name'] = '%s_%s_ACI_%s' % (templateVars['Workspace_Prefix'], templateVars['Site_Name'], folder)
                else:
                    kwargs['Workspace_Name'] = '%s_ACI_%s' % (templateVars['Site_Name'], folder)
                workspace_id = eval("%s.%s(**kwargs)" % (class_init, 'tf_workspace'))

                tf_folder_id = 'Site_ID_%s_%s' % (templateVars['Site_ID'], folder)
            ...
            for folder in folder_list:
                ...
                for var in var_list:
                    ...
                    kwargs['HCL'] = 'false'
                    if var == 'aciUrl':
                        kwargs['Var_Value'] = 'https://%s' % (templateVars['APIC_URL'])
...
def process_check_tf_cloud_vars(terraform_cloud_token, sensitive_var, **templateVars):
    ...
    TF_VAR_terraform_cloud_token = 'Bearer %s' % (terraform_cloud_token)
    MyHeaders = {'Authorization': TF_VAR_terraform_cloud_token,
               'Content-Type': 'application/vnd.api+json'
    }
    ...
    if not key_count == 1:
        ...
        post(uri, terraform_cloud_token, payload, template_file)
    exit()
